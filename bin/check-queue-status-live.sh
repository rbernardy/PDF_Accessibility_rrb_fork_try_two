...
#
# Options:
#   --bucket NAME       S3 bucket name (default: pdfaccessibility-pdfaccessibilitybucket149b7021e-tcuthkjujq0m)
...
if [ "$FAILURE_ANALYSIS_COUNT" -gt 0 ] && [ -n "$FAILURE_ANALYSIS_FILES" ]; then
    ...
    #     echo "  [DEBUG] WRITE_UNIQUE_FAILED_ANALYSIS='$WRITE_UNIQUE_FAILED_ANALYSIS'"
    #     WUFA_TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
    #     WUFA_FILE="./unique_failed_analysis_${WUFA_TIMESTAMP}.txt"
    #     
