...
if [ $CDK_EXIT_CODE -eq 0 ]; then
    ...
    notify_completion "success" "Test deployment completed successfully"
else
    echo ""
    echo "❌ TEST deployment FAILED with exit code $CDK_EXIT_CODE"
    notify_completion "failure" "Test deployment failed"
    exit $CDK_EXIT_CODE
fi
